...
//ReflectiveLoader = sub_140001000
ULONG_PTR sub_140001000(LPVOID dllBuffer) {
...
//TLS Callback = sub_140002000
void NTAPI sub_140002000(PVOID hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    if (ul_reason_for_call == DLL_PROCESS_ATTACH) {
        ...
        if (shellcode[0] == 0x4D && shellcode[1] == 0x5A) {
            //printf("MZ signature check complete! Valid PE file~!\n");
            ...
            //printf("Wait, that's not an MZ signature? Totally invalid!\n");
            //printf("Check it! First bytes is 0x%02X 0x%02X\n", shellcode[0], shellcode[1]);
            return;
        }
...
PIMAGE_TLS_CALLBACK p_tls_callback = sub_140002000;
