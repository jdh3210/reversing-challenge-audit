...
| **REV-03** | 셸코드 복호화 키가 코드 무결성에 묶여 있지 않음 — 안티디버깅 분기를 패치하면 키가 복원됨 | `PEB.BeingDebugged` 패치 또는 `key1` 상수 패치로 안티디버깅 무력화 | 중간 | *미해결* (§9) |
| **REV-05** | 복호화 이후 DLL이 RWX 메모리에 평문으로 상주 | TLS 콜백 종료 후 어태치 → 메모리 덤프로 DLL 획득 | 중간 | *미해결* (§9) |
...
- **그렇다면 해시를 바이너리에 둔 것이 오프라인 공격을 열어주지 않는가** — 오프라인 공격은 가능하다. 다만 그 공격의 내용이 곧 *정답 비밀번호를 맞히는 것*이 된다. `FLAG_HASH`는 입력이 아니라 **플래그**의 해시이므로, 공격자가 취할 수 있는 절차는 `입력 후보 → 키스트림 → 복호화 → 해시 비교`뿐이다. 그리고 비교를 통과하는 순간 손에 들려 있는 것이 이미 복호화된 플래그다. **해시를 깨는 것과 문제를 푸는 것이 같은 일이 되도록** 설계한 것이며, 이것이 목표한 상태다.
  보안의 근거는 해시의 강도가 아니라 비밀번호의 추측 난이도에 있다. 로컬에서 바이너리를 통제하는 공격자는 프로그램이 수행하는 검증을 어차피 그대로 재현할 수 있으므로, 해시를 숨긴다고 해서 얻는 것이 없다.
...
## 7. REV-02 — 미끼가 플래그 포맷을 알려주던 문제 <span class="lb lb-f">사실</span>

### 7-1. 문제

미끼 비밀번호를 입력하면 가짜 플래그가 출력됐다.
...
`strings`로 `EASY_REVERSING`을 발견해 입력하면 `DH{too_easy_password}`가 출력된다. 이 한 번으로 **플래그 포맷이 `DH{...}`임이 확정**되고, 이후에는 `DH{` 문자열 검색만으로 진짜 플래그를 겨냥할 수 있다.

미끼 자체는 의도한 장치였지만, **보상을 준 것이 문제였다.** 틀린 입력에 출력을 돌려주는 순간 그것은 단서가 아니라 오라클이 된다. 평문 플래그가 남아 있던 REV-01과 결합하면 `strings | findstr "DH{"` 한 줄로 챌린지가 끝난다.

### 7-2. 수정

미끼 문자열은 **단서로서 `.rdata`에 그대로 남기고**, 실제 비교만 실패하게 만들었다. 이미 설치한 IAT 후킹을 두 번째 목적으로 재사용하는 구조다.
...
`main()`의 분기와 `DH{too_easy_password}` 문자열은 **전혀 건드리지 않았다.** 수정은 DLL 쪽 후킹 함수 한 곳에만 들어간다.

```
v1   EASY_REVERSING  ->  Wow, Are you genius? flag is DH{too_easy_password}
v2   EASY_REVERSING  ->  Wrong~
```

풀이자는 여전히 `EASY_REVERSING` 문자열을 발견하고 시도해볼 수 있지만, 돌아오는 것은 오답 처리뿐이다. **단서는 유지하고 보상만 제거**해, 미끼가 오라클로 기능하지 않게 했다. (실행 화면은 §8 그림 5)
...
이 대조를 처음 실행했을 때 ④와 ⑤가 **동시에 실패**했다. 소스는 정상이었고 원인은 빌드 구성 하나였다. 보안 기능이 바이너리 레이아웃에 의존할 때 어떻게 무성으로 사라지는지를 보여주는 사례여서 별도로 기록했다 → [부록 A](#appendix-a)
...
## 부록 A. 빌드 파이프라인과 `/MD` 의존성 <span class="lb lb-f">사실</span> {#appendix-a}
...
이 항목을 남기는 이유는 가정이 아니라 **v2 검증 과정에서 실제로 발생한 상태**이기 때문이다. 수정을 마치고 실행했더니 미끼 입력이 여전히 가짜 플래그를 출력했고, 정답을 넣어도 `Wrong~`만 나왔다. 보안 수정 두 건이 동시에 무력화된 상태였는데 **빌드도 실행도 아무 오류를 내지 않았다.** 원인은 소스가 아니라 빌드 옵션 하나였다.
